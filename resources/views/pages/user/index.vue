    ...
    <div class="p-2 col-md-3 col-sm-12" v-for="(payment_method, key) in payment_methods" :key="key">
      <div class="card shadow-lg radius-20">
        <div class="card-body">
          <div class="row align-items-center">
            <div class="col-8">
              <span class="text-muted mb-3 lh-1 d-block text" style="text-transform:capitalize;"
                >{{payment_method.type}}</span
              >
              <h4 class="mb-3">
                <span class="counter-value" data-target="{{ payment_method.account }}">
                  {{ payment_method.account }} {{payment_method.symbol}}</span
                >
              </h4>
              <inertia-link
            :href="route('user.deposits.create')"
          ...
          ></inertia-link>
            ...
            <div class="col-4">
              <img :src="`/storage/payment_methods/${payment_method.svg}`" alt="" class="img-fluid" style="width:100px;"/>
            </div>
          </div>
        </div>
      </div>
...
<script setup>
  ...
  const fetchRates = () => {
    let urls = [];
    Object.values(payment_methods.value).forEach(val => {
      urls.push(`https://api.coingecko.com/api/v3/simple/price?ids=${val.type}&vs_currencies=usd`)
      axios.all(urls.map((price) => axios.get(price))).then(
